void receiveRemote(){
  // Serial.println("Attempt to receive"); // David
  if(millis() - lastReceivedMillis >= recDelay){
    ...
    if(Serial1.available() > 0){ //== recFromRemote){
      Serial.println(recFromRemote.Joy1X); // David
...
void flywheelSpin() {
    ...
    ch5PWM = constrain(map(recFromRemote.Joy1X, 0,512,255,-255),-255,255);
  ...
  if(ch5PWM > -10 && ch5PWM < 10){
    ch5PWM = 0;
  } else if(ch5PWM > 10){
    map(ch5PWM, 10, 255, 0, 255);
  } else if(ch5PWM < -10){
    map(ch5PWM, -10, -255, 0, -255);
  }
...
void domeSpin() {
    ...
    domeRotation = map(recFromRemote.Joy1X, 0,512,-255,255);
  #else
    domeRotation = map(recFromRemote.Joy1X, 0,512,255,-255);
  #endif   
  ...
  if ((currentDomeSpeed<=-20) && (ControllerStatus == 0 && IMUStatus == 0)){
    ...
    Serial.println(abs(currentDomeSpeed));
    ...
    Serial.print("Condition 2 Speed: ");
    Serial.println(abs(currentDomeSpeed));
  }else {
    analogWrite(domeSpinPWM1, 0);
    analogWrite(domeSpinPWM2, 0);
 ...
 */
  ...
  #ifdef reverseS2S
    if(recFromRemote.Fwd == 0 || recFromRemote.Fwd == 2){
        joystickS2S = map(constrain(recFromRemote.Joy4X, 0 , 512), 0,512,maxS2STilt,-maxS2STilt); //- is  left, + is  right
    }else{
        joystickS2S = map(constrain(recFromRemote.Joy4X, 0 , 512), 0,512,-maxS2STilt,maxS2STilt); //- is  left, + is  right
    }
  #else
